...
// UnmarshalJSON implements json.Unmarshaler.
func (j *GetBaseReportResponseJson) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["status"]; raw != nil && !ok {
		return fmt.Errorf("field status in GetBaseReportResponseJson: required")
	}
