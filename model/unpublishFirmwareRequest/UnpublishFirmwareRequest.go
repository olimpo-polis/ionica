...
package unpublishFirmwareRequest
...
// UnmarshalJSON implements json.Unmarshaler.
func (j *UnpublishFirmwareRequestJson) UnmarshalJSON(value []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(value, &raw); err != nil {
		return err
	}
	if _, ok := raw["checksum"]; raw != nil && !ok {
		return fmt.Errorf("field checksum in UnpublishFirmwareRequestJson: required")
	}
